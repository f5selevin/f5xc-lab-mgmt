...
class Xcspecsecurity extends Course {
    ...
    async newStudent({ email, namespace: requestedNamespace, deploymentId, dep_id: depId, udfHost, ip, region, log }) {
        ...
        if (initialized.status === 'error') {
            log.warn({ operation: 'xcspecsecurity.initialize', initialized });
            return initialized;
        }
        ...
        log.info({
            operation: 'xcspecsecurity.initialize',
            hash,
            namespace,
        ...
        if (err) {
            log.warn({ operation: 'xcspecsecurity.newStudent.failed', email, namespace, failure: err });
            return err;
        }
        ...
        log.info({ operation: 'xcspecsecurity.newStudent.success', email, namespace, recreated });
        return this.db.data.students[hash];
    }
}

export default Xcspecsecurity;
